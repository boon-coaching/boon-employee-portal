...
export function ResourceSuggestion({ competencyArea, sessionThemes, label }: ResourceSuggestionProps) {
  ...
  return (
    <div
      onClick={handleClick}
      className="group flex items-start gap-3 p-4 bg-boon-lightBlue/20 border border-boon-blue/10 rounded-xl cursor-pointer hover:border-boon-blue/30 transition-all"
    >
      <div className="w-8 h-8 rounded-lg bg-boon-blue/10 flex items-center justify-center flex-shrink-0">
      ...
      <div className="flex-1 min-w-0">
        {label && (
          <p className="text-[10px] font-bold text-boon-blue uppercase tracking-widest mb-1">{label}</p>
        )}
        <p className="text-sm font-bold text-boon-text group-hover:text-boon-blue transition-colors line-clamp-1">
          {suggestion.title}
        </p>
      ...
      <svg className="w-4 h-4 text-gray-400 group-hover:text-boon-blue transition-colors flex-shrink-0 mt-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
