  ...
  if (loading) {
    return (
      <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="h-48 bg-boon-offWhite rounded-card animate-pulse" />
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 md:space-y-10 animate-fade-in">
      ...
      <header className="pt-2">
        <h1 className="text-3xl md:text-4xl font-extrabold text-boon-navy tracking-tight">Your Journal</h1>
        <p className="text-boon-charcoal/55 mt-2 text-base font-medium">Reflect on your coaching journey</p>
      </header>

      {/* Stats row */}
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-card p-4 border border-boon-charcoal/[0.08] shadow-sm text-center">
          <p className="text-2xl font-black text-boon-navy">{entries.length}</p>
          <p className="text-[10px] font-bold text-boon-charcoal/55 uppercase tracking-widest mt-1">Reflections</p>
        </div>
        <div className="bg-white rounded-card p-4 border border-boon-charcoal/[0.08] shadow-sm text-center">
          <p className="text-2xl font-black text-boon-blue">{weeksWithEntries}</p>
          <p className="text-[10px] font-bold text-boon-charcoal/55 uppercase tracking-widest mt-1">Weeks Active</p>
        </div>
        <div className="bg-white rounded-card p-4 border border-boon-charcoal/[0.08] shadow-sm text-center">
          <p className="text-2xl font-black text-boon-success">{entries.filter(e => e.is_shared_with_coach).length}</p>
          <p className="text-[10px] font-bold text-boon-charcoal/55 uppercase tracking-widest mt-1">Shared</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 text-xs text-boon-charcoal/55">
      ...
      {coachingThemes.length > 0 && (
        <div className="bg-boon-lightBlue/20 border border-boon-blue/10 rounded-btn p-4">
          <p className="text-xs font-bold text-boon-blue uppercase tracking-[0.18em] mb-2">Your Coaching Themes</p>
      ...
      {entries.length > 0 && (
        <section>
          <h2 className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-boon-charcoal/55 mb-4">Past Reflections</h2>
          {groupedEntries.map((group) => (
            <div key={group.weekLabel} className="mb-6">
              <h3 className="text-xs font-semibold text-boon-charcoal/55 mb-3">{group.weekLabel}</h3>
      ...
      {/* Empty state */}
      {entries.length === 0 && (
        <section className="bg-white rounded-card p-8 md:p-10 border border-boon-charcoal/[0.08] shadow-sm text-center">
          <div className="w-14 h-14 mx-auto mb-4 bg-boon-lightBlue rounded-card flex items-center justify-center">
            <svg className="w-7 h-7 text-boon-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
          </div>
          <h2 className="text-xl font-bold text-boon-navy mb-2">Start your journal</h2>
