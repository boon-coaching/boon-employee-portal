 ...
 *
 * The service-role key is needed so we can bypass RLS and upsert freely.
...
async function upsertEmployee(email: string, fields: Record<string, unknown>) {
  const { data, error } = await supabase
    .from('employee_manager')
    .upsert({ company_email: email, company_id: COMPANY_ID, ...fields }, { onConflict: 'company_email' })
    .select('id')
    .single();
  if (error) throw new Error(`employee upsert (${email}): ${error.message}`);
...
async function seedPersona3a() {
  ...
  const empId = await upsertEmployee(email, {
    first_name: 'Ava',
    last_name: 'ScaleTest',
  ...
  await supabase.from('welcome_survey_scale').upsert(
    {
      email,
      coaching_goals:
  ...
  await supabase.from('session_tracking').upsert(
    {
      employee_id: empId,
...
async function seedPersona3b() {
  ...
  await supabase.from('welcome_survey_baseline').upsert(
    {
      email,
      coaching_goals:
