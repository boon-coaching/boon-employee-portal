-- ============================================
-- Add grow_first_session survey type
-- This survey is shown after the first GROW coaching session
-- ============================================

-- Update the survey_type check constraint to include grow_first_session
ALTER TABLE survey_submissions
DROP CONSTRAINT IF EXISTS survey_submissions_survey_type_check;

-- Add the updated constraint with grow_first_session
ALTER TABLE survey_submissions
ADD CONSTRAINT survey_submissions_survey_type_check
CHECK (survey_type IN ('scale_feedback', 'scale_end', 'grow_baseline', 'grow_first_session', 'grow_midpoint', 'grow_end'));

-- Update the pending_surveys view to use grow_first_session for GROW session 1
...
milestone_surveys AS (
  SELECT
    ...
    -- GROW session 1 = grow_first_session
    -- GROW midpoint = grow_midpoint
    -- SCALE = scale_feedback
    CASE
      WHEN pi.program_type = 'GROW' AND st.appointment_number = 1
        THEN 'grow_first_session'
      WHEN pi.program_type = 'GROW' AND st.appointment_number = (pi.sessions_per_employee / 2)
        THEN 'grow_midpoint'
      WHEN pi.program_type = 'EXEC' AND st.appointment_number = 1
        THEN 'grow_first_session'
      WHEN pi.program_type = 'EXEC' AND st.appointment_number = (pi.sessions_per_employee / 2)
        THEN 'grow_midpoint'
      ELSE 'scale_feedback'
